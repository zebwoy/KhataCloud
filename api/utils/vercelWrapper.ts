...
export function vercelWrapper(netlifyHandler: Handler) {
  return async (req: any, res: any) => {
    ...
    // Get body string (Vercel automatically parses JSON bodies)
    let bodyStr = '';
    if (req.body) {
    ...
    try {
      ...
      if (result) {
        ...
        res.status(500).json({ error: 'No response from Netlify handler' });
      }
    } catch (err: any) {
